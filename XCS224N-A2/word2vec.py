...
def naiveSoftmaxLossAndGradient(
    ...
    ### YOUR CODE HERE
    #centerWordVec (3)  (embedding_dim)
    #outsideVectors  (5,3)  (vocab_size, embedding_dim)
    probs = softmax(np.dot(outsideVectors, centerWordVec))  # (embedding_dim, 1)
    loss = - np.log(probs[outsideWordIdx])

    probs[outsideWordIdx] -= 1 #compute probs gradient: ˆy − y ==> ˆy - 1 for outsideWordIdx, ˆy for the others
    gradCenterVec   = np.dot(probs, outsideVectors) # (embedding_dim,1)
    gradOutsideVecs = np.outer(probs, centerWordVec)  # (vocab_size, embedding_dim)
    ### END YOUR CODE
...
def skipgram(currentCenterWord, windowSize, outsideWords, word2Ind,
    ...
    for word in outsideWords:
        ...
        lossCurrent, gradc, grado = word2vecLossAndGradient(centerWordVec=centerWordVec, outsideWordIdx=wordIdx, outsideVectors=outsideVectors, dataset=dataset)
        loss += lossCurrent
        gradCenterVecs[center_id] += gradc
